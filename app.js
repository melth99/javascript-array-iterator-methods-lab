...
console.log('Exercise 1 correct result: ', [
 ...
 veryOldInventors= inventors.filter((person)=>{
...
inventorNames = inventors.map((man) => {
    return (`first: ${man.first}, last: ${man.last}: Last Name`)
});
...
firstLast = people.map((nameList) => {
    const [last, first] = nameList.split(', '); // Split by ', ' to separate last and first name
    return `${first} ${last}`; // Rejoin in "First Last" format
});
...
// Complete the exercise in the space below:
commentById = comments.find((note)=>{
  return note.id === 823423
})
